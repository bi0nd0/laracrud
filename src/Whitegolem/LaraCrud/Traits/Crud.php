<?php namespace Whitegolem\LaraCrud\Traits;
...
trait Crud {
	...
	{
		$attributes = $attributes ?: self::toArray();
	...
	{
		return isset(static::$searchable) ? static::$searchable : self::getFillable();
	}
	...
	*/
	public function updateRelations() {
		...
		foreach($keys as $key)
		{
			if ( method_exists(self, $key) && is_a(self::$key(),'Illuminate\Database\Eloquent\Relations\Relation') ) {
				$methodClass =  get_class(self::$key());

				switch($methodClass)
				{
					case 'Illuminate\Database\Eloquent\Relations\BelongsToMany':
						$data = $attributes[$key];
						self::setBelongsToMany($key,$data);
						break;
					case 'Illuminate\Database\Eloquent\Relations\HasOneOrMany':
					case 'Illuminate\Database\Eloquent\Relations\HasOne':
					case 'Illuminate\Database\Eloquent\Relations\HasMany':
						$data = $attributes[$key];
						self::setHasOneOrMany($key,$data);
						break;
				}
			}
		}
	...
	**/
	protected function setBelongsToMany($relationCallable = false, $data = array())
	{
		...
		$relatedCollection = self::$relationCallable()->get(); //Collection of attached models
		...
		foreach($data as $attachableModelInput)
		{
			...
			$relatedInstance = self::$relationCallable()->getRelated(); //instance of the related model

			if( $attachableModel = $relatedInstance::find($attachableModelID) )
			{
				if($relatedCollection->contains($attachableModelID))
					self::$relationCallable()->detach($attachableModelID); //detach to update pivot data
				
				$pivotData = (isset($attachableModelInput['pivot'])) ? $attachableModelInput['pivot'] : array();
				self::$relationCallable()->attach($attachableModelID, $pivotData);
		...
		if (count($detachIds) > 0) self::$relationCallable()->detach($detachIds);
	}
	...
	**/
	...
	{
		$attached = self::$relation()->get(); //models già associati
		...
		foreach($data as $attachableData)
		{
			...
			$relatedInstance = self::$relation()->getRelated(); //instance of the related model
			if($attachable = $relatedInstance::find($attachableID))	$attachables[] = $attachable;
		}
		self::$relation()->saveMany($attachables);

		//detach related model not sent
		$attached->each(function($item) use($relation, $attachables){
			if( ! in_array($item, $attachables) ) self::detachModel($relation, $item);
		});
	}
	...
	*/
	...
	{
		$foreignKey = self::$relation()->getPlainForeignKey();
